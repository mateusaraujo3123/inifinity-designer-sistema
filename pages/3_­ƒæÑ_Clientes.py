...
with tabs[0]:
    ...
    if projetos_c.empty:
        ...
        programas_df = read_df("Programas")
            ...
            with st.expander(f"📁 {proj['nome_projeto']} — {proj['descricao'] or 'sem descrição'}"):
                ...
                if categorias.empty:
                    ...
                    with st.form(f"nova_arte_{proj['id']}", clear_on_submit=True):
                        cat_id = st.selectbox(
                            "Tipo de arte (categoria)", options=list(cat_dict.keys()),
                        ...
                        progs_sel = st.multiselect(
                            "Programas utilizados", options=prog_opcoes, key=f"progs_{proj['id']}",
                        )
                        prazo = st.date_input("Prazo de entrega", key=f"prazo_{proj['id']}")
                        anotacoes = st.text_area("Anotações", key=f"anot_{proj['id']}")
